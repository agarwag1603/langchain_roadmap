...
#                 retriever = vectorstore.as_retriever(search_type="mmr", search_kwargs={"k": int(search_arguments), "lambda_mult": 0.5})
#             else:
...
#                 st.subheader(f"Top {search_arguments} results:")
#                 for i, doc in enumerate(response, start=1):
#                     st.markdown(f"**Result {i}:** {doc.page_content[:300]}...")  # Truncate text
#             else:
#                 response = compression_retriever.invoke(user_query)
#                 st.subheader(f"Top {search_arguments} results:")
#                 for i, doc in enumerate(response, start=1):
#                     st.markdown(f"**Result {i}:** {doc.page_content[:300]}...") 
